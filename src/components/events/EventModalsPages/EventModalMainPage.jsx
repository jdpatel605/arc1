...
import { createEventRequest, editEventRequest } from "../../../store/actions/eventActions";
...
const EventModalMainPage = () => {
  ...
  const { eventCreateLoading, eventCreateError, eventCreateFlag } = useSelector(({ events }) => events);
  const {
    eventDetails, setEventDetails, setCurrentPage, handleHideEventModal, activeTab, eventEditMode, editEventId,
    validationErrors, setValidationErrors, setActiveTab, showEventModal, createFrom
  ...
  useEffect(() => {
    if (showEventModal === true) {
      ...
      if (eventDetails.eventGroupHost.id !== '') {
        validateField('eventGroupHost', eventDetails.eventGroupHost.id);
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (eventEditMode === true) {
        ...
        if (eventDetails.eventName === '' && eventDetails.eventGroupHost.id === '' && eventDetails.date === '' && (Object.keys(eventDetails.startTime).length === 0 && eventDetails.startTime.constructor === Object) && (Object.keys(eventDetails.endTime).length === 0 && eventDetails.endTime.constructor === Object)) {
          validationErrors.eventName = {};
          validationErrors.eventGroupHost = {};
    ...
    try {
      ...
      if (field === 'eventName') {
        if (value.trim() === '') {
          ...
          setValidationErrors(prevProps => ({
            ...prevProps, eventName: { valid: false, message: 'Event Name must contain letter also' }
          }));
        } else {
          validationErrors.eventName = { valid: true, message: '' }
          // setValidationErrors(prevProps => ({ ...prevProps, eventName: { valid: true, message: '' } }));
        }
      }

      // Validate event host
      if (field === 'eventGroupHost') {
        if (eventDetails.eventGroupHost.id === '') {
          setValidationErrors(prevProps => ({
            ...prevProps, eventGroupHost: { valid: false, message: 'Event Host is required' }
          }));
        } else {
          // setValidationErrors(prevProps => ({ ...prevProps, eventGroupHost: { valid: true, message: '' } }));
          validationErrors.eventGroupHost = { valid: true, message: '' }
        }
      }
      ...
      if (field === 'date') {
        if (value === null) {
          ...
          validationErrors.date = { valid: true, message: '' }
        ...
        if (eventDetails.startTime.value) {
          ...
          if (startTimeDiff < 0) {
            ...
            validationErrors.startTime = { valid: true, message: '' }
      ...
      if (field === 'endTime') {
        let startTimeSelect = "00:00:00";
        let endTimeSelect = value;
        if (eventDetails.startTime.value) {
          startTimeSelect = eventDetails.startTime.value
        }
        // calculate total duration
        /* var timeStart = new Date("01/01/2007 " + startTimeSelect).getMinutes();
        var timeEnd = new Date("01/01/2007 " + endTimeSelect).getMinutes();
        var minutesDiff = timeEnd - timeStart;
        ...
        if (value === '') {
          ...
          validationErrors.endTime = { valid: true, message: '' }
          // setValidationErrors(prevProps => ({ ...prevProps, endTime: { valid: true, message: '' } }));
        }
      }
  ...
  const allValidateField = (value) => {
    ...
    if (eventDetails.eventGroupHost.id === '') {
      validationErrors.eventGroupHost = { valid: false, message: 'Event Host is required' }
      setShowErrors(true);
    }
    ...
    if (value.eventName !== '' && eventDetails.eventGroupHost.id !== '' && eventDetails.date !== '' && (Object.keys(eventDetails.startTime).length !== 0 && eventDetails.startTime.constructor === Object) && (Object.keys(eventDetails.endTime).length !== 0 && eventDetails.endTime.constructor === Object)) {
      ...
      const payload = {
        ...
        data: {
          identifier: eventDetails.identifier,
  ...
  const handleAddTimeSlots = (type = 'startTime', timeValue = '00:00:00', date = '', label = '') => {
    try {
      ...
      if (minuteBreathSlot >= 15) {
        ...
        let currentDateObj = moment(dateValue).format(`YYYY-MM-DD 00:00:00`);
        const timeSlots = [];
        if (nearestMinute === 55) {
        ...
        const nextDayObj = moment(dateValue).add('1', 'days').format('YYYY-MM-DD 00:00:00');
        ...
        for (let index = 1; index <= 100; index++) {
          const checkTime = currentDate.format(`HH`);
        ...
        if (type === 'startTime') {
          /* if(date !== currentDateStr) {
            let prevTimeStart = moment(dateValue).format(`YYYY-MM-DD 00:00:00`);
            let prevTimeEnd = moment(dateValue).format(`YYYY-MM-DD 07:00:00`);
            const startPrevTime = moment(prevTimeStart);
            const endPrevTime = moment(prevTimeEnd);
            let diffLabel = '';
            for (let index = 1; index <= 100; index++) {
              const checkTime = startPrevTime.format(`HH`);
              if (startPrevTime < endPrevTime) {
                timeSlots.push({
                  value: startPrevTime.format(`HH:mm:00`),
                  label: startPrevTime.format(`hh:mm a`) + diffLabel
                });
                startPrevTime.add('15', 'minutes');
              }
            }
          } */
          setEventStartTimeOptions(timeSlots);
        } else {
          if(timeSlots && timeSlots[1]) {
            setEventDetails(preProps => ({ ...preProps, endTime: timeSlots[1] }));
            setValidationErrors(prevProps => ({ ...prevProps, endTime: { valid: true, message: '' } }));
          }
  ...
  const focusOccurances = () => {
    if (occurances === '') {
      setOccurances(' ');
    }
  }

  const focusOutOccurances = () => {
    if (occurances === ' ') {
      setOccurances('');
    }
  }
  ...
  return (
    <>
      <div className="modal-header justify-content-between pt-0">
        <div>
          <h4 className="modal-title">{eventEditMode === false || eventCreateFlag === 1 ? 'Create' : 'Edit'} Event</h4>
            ...
            <div id="collapse-A" className="collapse" data-parent="#content" role="tabpanel" aria-labelledby="screen-heading-A">
              <form className="form-sec" noValidate="" onSubmit={(e) => handelCreateGroup(e)}>
                    ...
                    <div className="form-group">
                      <div className="bg-black-500">
                        <textarea
                          ...
                          maxLength="500"
                        />
                        <label className="txt-label">Event Description (opt)</label>
                      </div>
                    </div>
                    <p>Event Host</p>
                    <div className="form-group">
                      {createFrom && createFrom === 'admin' &&
                      <div className={`bg-black-500 setting-box`}>
                      ...
                      {!createFrom &&
                      <>
                        <div className={`bg-black-500 setting-box ${validationErrors.eventGroupHost.valid === false && 'err-div'}`} onClick={() => showGroupHostPage()}>
                          <span className={`setting-text ${eventDetails.eventGroupHost.id && 'text-white font-weight-bold'} `}>{eventDetails.eventGroupHost.name}</span>
                          {
                            (eventEditMode === false) &&
                        ...
                        {validationErrors.eventGroupHost.valid === false && displayErrorMessage(validationErrors.eventGroupHost.message)}
                      </>
                      }
                    </div>
                    <p>Event Details</p>
                ...
                <div className="btn-sec pl-2 pr-2">
                  <span className="btn btn-medium md-box btn-black clr-white" onClick={() => hideModal()}>Cancel</span>
                  <button type="button"
                    onClick={handelCreateGroup}
                    className={`btn btn-medium md-box btn-submit btn-green`}
                    disabled={eventCreateLoading}
                    // className={`btn btn-medium md-box btn-submit ${validForm ? 'btn-green' : 'btn-gray-500'} `}
                    // disabled={!validForm || eventCreateLoading}
                  >
                    {eventEditMode === false || eventCreateFlag === 1 ? 'Create Event' : 'Save'} {eventCreateLoading === true && <Spinner />}
                  </button>
                </div>
              </form>
            </div>
