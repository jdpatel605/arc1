...
import {createGroupRequest, updateGroupRequest, organizationListRequest} from './../../store/actions/group';
import ImageCrop from "../common/ImageCrop";
import {Logger} from './../../utils/logger';
const fileLocation = "src\\components\\group\\CreateGroup.jsx";

const CreateGroup = props => {
  const dispatch = useDispatch();
  const {createFlag, createFlagLoading, createGroupError, updateFlag, updateFlagLoading, updateGroupError, orgList} = useSelector(({group}) => group);
  ...
  useEffect(() => {
    if(props.organizationId !== 'null') {
      ...
      setDefaultOrg({label: props.organizationName, value: props.organizationId});
  ...
  useEffect(() => {
    ...
    try {
      ...
      Logger.error({fileLocation, message, trace: 'useEffect:createFlag'})
    }
  }, [createFlag]);
  ...
  useEffect(() => {
    if(props.is_edit) {
      ...
      if(props.editData.organization) {
        setGroupValues({
          organizationId: props.editData.organization.identifier,
          groupName: props.editData.name,
          groupDescription: props.editData.description
        })
        setOrganizationName(props.editData.organization.name);
        setDefaultOrg({label: props.editData.organization.name, value: props.editData.organization.identifier});
        setIsDisabled(true);
      }
      ...
      if(orgList && orgList.eventData && orgList.eventData.entries.length > 0) {
        ...
        if(cutomvalue && cutomvalue.length === 1) {
          setOrganizationName(cutomvalue[0].label);
          setDefaultOrg({label: cutomvalue[0].label, value: cutomvalue[0].value});
          setGroupValues({organizationId: cutomvalue[0].value, groupName: '', groupDescription: ''});
        }
        else {
          setDefaultOrg([]);
        }
  ...
  useEffect(() => {
    try {
      if(createGroupError.message) {
        if(createGroupError.message === "name has already been taken") {
      ...
      Logger.error({fileLocation, message, trace: 'useEffect:createGroupError'})
    }
  }, [createGroupError]);
  ...
  useEffect(() => {
    try {
      if(updateGroupError.message) {
        if(updateGroupError.message === "name has already been taken") {
          setUniqueGroup('Group name has already been taken');
        } else {
          setUniqueGroup(null);
        }
      ...
      Logger.error({fileLocation, message, trace: 'useEffect:updateGroupError'})
    }
  }, [updateGroupError]);
  ...
  const handelCreateGroup = data => {
    ...
    if(editData.group_id !== '') {
      ...
      dispatch(updateGroupRequest(formData));
    } else {
      ...
      <Modal size="lg" show={props.show} backdrop="static"
        ...
        <Modal.Footer className="justify-content-between">
          ...
          <button type="button" className={`btn btn-green md-box btn-medium ${!validForm && 'btn-gray-500'}`}
            ...
            disabled={!validForm || createFlagLoading || updateFlagLoading}>Continue {(createFlagLoading || updateFlagLoading) && <Spinner />} </button>
        </Modal.Footer>
      </Modal>
      <ImageCrop
        image={cropImage}
